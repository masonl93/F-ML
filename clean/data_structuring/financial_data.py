...
class FinDataStruct():
    def __init__(self):
        ...
        # In addition, set class variables for volume, dollar volume column names
        # so they can be used in get_dollar_bars and get_volume_bars instead of
        # hard-coded
        '''
        pass

    def load_ticks_CSV(self, fp):
        """[summary]
    ...
    def df_to_parquet(self, df, out_fp):
        df.to_parquet(out_fp)

    def parquet_to_df(self, out_fp):
        return pd.read_parquet(out_fp)

    def get_tick_bars(self, df, tick_threshold):
        return df[::tick_threshold]

    def get_volume_bars(self, df, volume_threshold):
        idxs = []
        vol_counter = 0
    ...
    def get_dollar_bars(self, df, dollar_threshold):
        idxs = []
        dol_counter = 0
        # for i, j in enumerate(df['self.volume_col'])
    ...
    def get_tick_imbalance_bars(self, df, exp_tick_imbal=100):
